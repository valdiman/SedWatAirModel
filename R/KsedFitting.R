...
# -------------------------
# Congener-specific block (CHANGE ONLY HERE for other PCBs)
# -------------------------
...
# -------------------------
# 5) Objective: SSE between modeled mpuf and observed mpuf_obs at obs times
# -------------------------
...
cat("Best-fit ksed (1/day):", signif(ksed_best,6), "\nSSE:", signif(res$value,6), "\n\n")

# -------------------------
# 7) Final run & comparison
# -------------------------
...
# -------------------------
# 8) Mass-balance diagnostics (ng)
# -------------------------
Vpuf_L <- 0.029; Vpw_L <- 0.004; Vw_L <- 0.100; Va_L <- 0.125
df_diag <- df_best %>%
  mutate(ms_mass = Cs * parms_base$ms,
         mpw = Cpw * Vpw_L,
         mw  = Cw  * Vw_L,
         ma  = Ca  * Va_L,
         mpuf = mpuf) %>%
