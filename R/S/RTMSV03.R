...
rtm.PCBi <- function(t, state, parms) {
  ...
  # Parameters
  with(parms, {
    Cs <- state[1]; Cw <- state[2]; Ca <- state[3]; Cpuf <- state[4]
    ...
    list(c(dCsdt, dCwdt, dCadt, dCpufdt))
  })
}
...
final_out_df <- ode(y = cinit, times = t.1, func = rtm.PCBi, parms = best_parms) %>%
  ...
  mutate(
    mpuf = Cpuf * 29 / 1000,
    Mt = (Cs * 10 / (0.1 * 1000)) + (Cw * 0.1) + (Ca * 0.125) + mpuf
  )
