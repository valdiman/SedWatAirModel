...
cost_func_massbalance <- function(parms) {
  parms_list <- list(
    ro = parms["ro"],
    ko = parms["ko"],
    kdf = parms["kdf"],
  ...
  # Observed data (SPME and PUF)
  obs <- obs.data.pcbi.2 %>% filter(time %in% out_df$time)
  
  # Mass balance error: Cs * 0.01 should equal (Mt - other compartments)
  Mt <- (out_df$Cs * 0.01) + (out_df$Cw * 0.1) + pred_spme + (out_df$Ca * 0.125) + pred_puf
  mass_error <- (out_df$Cs * 0.01) - (Mt[1] - (out_df$Cw * 0.1 + pred_spme + out_df$Ca * 0.125 + pred_puf))
  
  # Combine residuals (observations + mass balance)
  c(
    pred_spme - obs$SPME,    # SPME residuals
    pred_puf - obs$PUF,      # PUF residuals
    mass_error               # Mass balance residuals
  )
}

# 5. Parameter fitting
par_guess <- c(ro = 540, ko = 10, kdf = 1.9, kds = 0.001, f = 0.8, ka = 150)
lower_bounds <- c(ro = 450, ko = 1e-3, kdf = 1e-3, kds = 1e-6, f = 0, ka = 1)
upper_bounds <- c(ro = 600, ko = 1e3, kdf = 10, kds = 1, f = 1, ka = 1e3)

# Fit with mass balance constraint
fit_massbalance <- modFit(
  f = cost_func_massbalance,
  p = par_guess,
  lower = lower_bounds,
  upper = upper_bounds
)
...
# See parameters
summary(fit_massbalance)

# 6. Extract best parameters
best_parms_massbalance <- as.list(fit_massbalance$par)
best_parms_massbalance$kb <- 0  # No biodegradation

# 7. Run final model
fine_time <- seq(from = min(t.1), to = max(t.1), by = 0.5)  # 0.5-day intervals

final_out_massbalance <- ode(
  y = cinit,
  times = fine_time,
  func = rtm.PCBi,
  parms = best_parms_massbalance
)

final_out_df_massbalance <- as.data.frame(final_out_massbalance) %>%
  mutate(
    mspme = Cspme * 6.9e-8,
    mpuf = Cpuf * 29 / 1000,
    Mt = (Cs * 0.01) + (Cw * 0.1) + mspme + (Ca * 0.125) + mpuf
  )

# Plot mass conservation
ggplot(final_out_df_massbalance, aes(time, Mt)) +
  geom_line(color = "red") +
  labs(title = "Mass Conservation Check (After Fitting)", x = "Time (days)", y = "Total Mass (ng)")

# SPME Plot
spme_plot <- ggplot() +
  geom_line(data = final_out_df_massbalance, 
