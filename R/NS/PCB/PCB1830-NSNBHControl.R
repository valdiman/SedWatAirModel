...
# Read data ---------------------------------------------------------------
{
  exp.data <- read.csv("Data/01_NS_SPME_PUF.csv", stringsAsFactors = FALSE,
                       check.names = FALSE)
...
# Organize data -----------------------------------------------------------
{
  ...
  pcb_combined_control <- rbind(
    data.frame(time = 0, mf_control = 0,
               mpuf_control = 0),
...
# Comparison and plot
{
  ...
  # Export data
  #write.csv(model_results_daily_clean, file = "Output/Data/NS/PCB1830ControlNBH.csv")
...
# Save plot in folder
ggsave("Output/Plot/NS/PCB1830ControlNBH.png", plot = plot, width = 6,
       height = 5, dpi = 500)
